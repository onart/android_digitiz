...
|---|---|---|
| 좌표 | `x, y` | 그 자리를 클릭 |
| 영역 | `x, y, 너비, 높이` | 버튼이 그 영역의 축소판이 된다. 안을 누르면 대응 지점 클릭, 문지르면 드래그 |
| 단축키 | `ctrl+shift+s` | 그 키 조합을 PC에 보낸다 |

좌표는 **호스트 창의 `Cursor:` 줄**에서 읽는다. PC에서 원하는 지점에 마우스를
올리고 그 숫자를 폰에 입력하면 된다 (`Copy` 버튼도 있다).

들어갈 자리보다 버튼이 많으면 양 끝의 점 두 개로 **순환**한다. 스크롤이 아니라
순환인 이유는, 자리 수가 고정이면 같은 버튼이 볼 때마다 같은 자리에 있기
때문이다. 가로(아래 가장자리)와 세로(왼쪽 가장자리)는 사이드 메뉴 헤더의 막대
글리프 버튼으로 바꾼다. 방향과 접힘 상태는 폰에 저장된다.
