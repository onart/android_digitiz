...
- **텍스트는 안드로이드 `Paint` 로 래스터화해 GL 텍스처로 올린다** (`text/TextRenderer`).
  폰트를 번들하는 방식(stb_truetype)은 한글을 넣는 순간 CJK 폰트가 수 MB이고,
  시스템 폰트 경로는 OEM마다 다르다. 플랫폼에 맡기면 한글이 공짜로 되고 APK도
  가벼워진다. 대가는 좁은 JNI 표면 하나 — 그리고 프로젝트 전체에서 이것뿐이다.
  문자열+크기 조합마다 한 번 래스터화해 텍스처로 캐시하고, 알파 채널만 저장해
  색은 그릴 때 입힌다. 라벨은 `strings.xml` 에서 가져오므로 번역이 그대로 적용된다.
